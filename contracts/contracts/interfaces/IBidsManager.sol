...
interface IBidsManager {
  ...
  function getBidReturnAmount(uint256 bidId) external view returns (uint256);
  function getLoanPremiumIndex(uint256 loanId) external view returns (uint256);
  function getBid(uint256 bidId) external view returns (uint256, uint256, uint256);
  function isLoanFulfilled(uint256 loanId) external view returns (bool);
}